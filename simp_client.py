...
class Client:
    ...
    def send_name_to_daemon(self, name):
        try:
            self.daemon_tcp_socket.sendall(name.encode("utf-8"))
            print(f"Sent name to daemon: {name}")
        except Exception as e:
            print(f"Error sending name to daemon: {e}")
